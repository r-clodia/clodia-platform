...
if command -v sqlite3 &>/dev/null; then
    ...
    echo "Applicazione schema leads.db..."
    mkdir -p "$DATADIR/data/tomato"
    sqlite3 "$DATADIR/data/tomato/leads.db" < "$BUNDLE_ROOT/tools/app/tomato_leads/schema.sql"
else
...
# trusted.json per WhatsApp (vuoto — da popolare con il LID di Davide)
