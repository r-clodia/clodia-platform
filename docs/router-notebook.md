...
## R16 · The secretary keeps a narrow profile and gains a conditional duty
...
That makes R16 small where it looked large: one more `kind`, one more directive, and a mandate
in the seed that reads it.

### The third outcome, which answers an open point

R10 left «what does the coordinator return» open. R16 answers it with **three** outcomes, not
two:
...
2. somebody else is the right agent → it **mentions** them, and the mention is the hand-over —
   which is R2's direct route, reached from inside the room instead of from a person;
...
also closes the loop with R14: «add an agent to the scope» is exactly the act that R14 governs,
